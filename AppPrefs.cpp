...
bool Load() {
    ...
    char* name = ToUtf8Temp(GetComputerName());
    if (!str::EqI(gprefs->computerName,name)) {
        str::ReplaceWithCopy(&gprefs->computerName, name);
    } else {
        newComputer = false;
    }
